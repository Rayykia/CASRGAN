...
class AHPF(nn.Module):
    ...
    def forward(self, x):
        self.hp_factor.data = self.hp_factor.clamp(0.01, 0.11)
        smooth_x = x - F.conv2d(x, self.weight * self.hp_factor, padding = self.padding)
...
class CARB(nn.Module):
    ...
    def forward(
        ...
        x = self.conv1(x + conv_path)
        x = self.se(x)
        x = (self.conv2(x) + global_identity) * 0.1
        return x
...
class CASRModel(nn.Module):
    def __init__(
        ...
        # residual learning
        self.res_branch0 = nn.ModuleList([
            CARB(
        ...
        self.res_branch1 = CARB(
                io_nc=io_nc, dim=dim, patch_size=patch_size, n_stb=6, input_size=input_size,
                num_heads=num_heads, window_size=window_size, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                qk_scale=qk_scale, attn_drop=attn_drop, drop=drop, norm_layer=norm_layer,
                norm_before=norm_before, deepnorm=deepnorm, n_layers=n_layers
            )
        
        self.res_branch2 = CARB(
                io_nc=io_nc, dim=dim, patch_size=patch_size, n_stb=4, input_size=input_size,
                num_heads=num_heads, window_size=window_size, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                qk_scale=qk_scale, attn_drop=attn_drop, drop=drop, norm_layer=norm_layer,
                norm_before=norm_before, deepnorm=deepnorm, n_layers=n_layers
            )
    ...
    def forward(
        ...
        x = self.sfe(x)
        shortcut = x
        for carb in self.res_branch0:
            x = carb(x)
        x = self.res_branch1(x)
        x = self.res_branch2(x)
...
# class CASRDiscriminator(nn.Module):
#     def __init__(self, *args, **kwargs) -> None:
#         super().__init__(*args, **kwargs)
#         pass

# class CASRGANModel(BaseGAN):
