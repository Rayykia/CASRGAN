...
def default_conv(
    ...
    Parameters
        input_nc (int)          -- the number of channels in the input feature
        output_nc (int)         -- the number of channels in the output feature
...
class ResBlock(nn.Module):
    def __init__(
        ...
        Parameters:
            conv_layer (Callable[...,nn.Conv2d])        -- the convolution layer used
            ngf (int)                                   -- the number of filters within one convolution layer
            kernel_size (int)                           -- the kernel size
            bias (bool)                                 -- use bias `default: True`
            norm_layer (str)                            -- the normalization layer: [batch_norm | instance_norm | none] `default: 'none'`
            act_layer (nn.Module)                       -- activation function `default: nn.ReLU()`
            res_scale (float)                           -- residual scaling factor, 0.1 was used in EDSR `default: 1`
        """
...
class Upsampler(nn.Module):
    def __init__(
        ...
        Parameters:
            conv_layer (nn.Conv2d)          -- the convolution layer used
            scale (int)                     -- upsampling scale 
            ngf (int)                       -- the number of filters within one convolution layer
            norm_layer (str)                -- the normalization layer: [batch_norm | instance_norm | none] `default: 'none'`    
            bias (bool)                     -- use bias `default: True`
            act (str)                       -- activation layer [relu | prelu]
        """
        super().__init__()
        modules = []
...
class _RDB_conv(nn.Module):
    def __init__(
        ...
        Parameters:
            input_nc (int)              -- # of input channel of the layer
            growth_rate (int)           -- # of output channels of the layer
...
class ResidualDenseBlock(nn.Module):
    def __init__(
        ...
        Parameters:
            growth_rate_0 (int)             -- # of input and output channels of the RDB
            growth_rate (int)               -- # of the output channels of the inner convolution layers
            n_conv_layers (int)             -- # of convlution layers used within an RDB, referenced as 'C' in the original paper
            k_size (int)                    -- kernel size of the inner convolution layers
        """
        super().__init__()
...
class _DRRNResidualUnit(nn.Module):
    def __init__(
        ...
        Parameters:
            ngf (int)       -- # of filters in the conv-layers
    ...
    def forward(
        ...
        Parameters:
            h_0 (Tensor)        -- output of the 1st conv-layer within the recursive block
            h_in (Tensor)       -- input of the residule unit
        """
...
class RecursiveBlock(nn.Module):
    def __init__(
        ...
        Parameters:
            n_res (int)             -- # of residule units within one recursive block, referenced as 'U' in the original paper
